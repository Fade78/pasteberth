...
and integrating Pasteberth 2.1.16. The short project overview is in
...
The 2.1.16 implementation requires:
...
Linux is the current official and tested server platform for v2.1.16. The
...
The supported v2.1.16 installation is the tracked `PasteBerth/` directory. It is
...
| `type` | `local` | Only `local` is implemented in v2.1.16. |
...
the v2.1.16 support matrix and must not be represented as already supported.
