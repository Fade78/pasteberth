...
The 2.1.25 implementation requires:
...
Linux is the current official and tested server platform for v2.1.25. The
...
The supported v2.1.25 installation is the tracked `PasteBerth/` directory. It is
...
```sh
git clone --branch v2.1.25 --depth 1 https://github.com/Fade78/pasteberth.git
cd pasteberth
...
public proxy. The `items` alias is **Unreleased, after 2.1.25**; blocking only
...
the v2.1.25 support matrix and must not be represented as already supported.
